import copy, logging, shutil, uuid
...
from utils import build_empty_mask_manifest, save_single_video_mask_frame, write_mask_manifest
from video.io import build_window_dir
from video.mask_commit import commit_pending_masks_dir, prepare_pending_masks_dir
...
def _resolve_propagation_frame_range(
    ...
    if request.start_frame_idx is not None:
        ...
        start_frame_idx = min(int(event["frame_idx"]) for event in prompt_events)
    start_frame_idx = min(max(start_frame_idx, 0), num_frames - 1)

    if request.end_frame_idx is not None:
        end_frame_idx = min(max(int(request.end_frame_idx), 0), num_frames - 1)
        if end_frame_idx < start_frame_idx:
            raise HTTPException(status_code=400, detail="end_frame_idx must be >= start_frame_idx.")
    elif request.max_frame_num_to_track is None:
        end_frame_idx = num_frames - 1
    else:
        requested = int(request.max_frame_num_to_track)
        if requested <= 0:
            return start_frame_idx, start_frame_idx - 1
        end_frame_idx = min(num_frames - 1, start_frame_idx + requested - 1)

    return start_frame_idx, end_frame_idx

def _snapshot_propagation_state(request: VideoPropagateRequest) -> PropagationSnapshot:
    with state.video_state_lock:
        ...
        return PropagationSnapshot(
            ...
            video_source_path=state.video_source_path,
...
def run_propagation_job(request: VideoPropagateRequest) -> dict[str, Any]:
    ...
    snapshot = _snapshot_propagation_state(request)
    effective_online_mode = snapshot.online_mode
    effective_batch_size = snapshot.batch_size
    effective_offload_video_to_cpu = snapshot.offload_video_to_cpu
    effective_offload_state_to_cpu = snapshot.offload_state_to_cpu

    num_frames = len(snapshot.frame_files)
    start_frame_idx, end_frame_idx = _resolve_propagation_frame_range(request, num_frames, snapshot.prompt_events)
    if end_frame_idx < start_frame_idx:
        return {
            ...
            "propagation_end_frame_idx": end_frame_idx,
            "state.mask_manifest_path": snapshot.mask_manifest_path,
    ...
    pending_masks_dir = prepare_pending_masks_dir(snapshot.session_dir)
    frame_files = [Path(snapshot.video_dir) / frame_file for frame_file in snapshot.frame_files]
    masks_dir = pending_masks_dir
    manifest_file_path = masks_dir / "manifest.json"

    first_frame = cv2.imread(str(Path(snapshot.video_dir) / snapshot.frame_files[start_frame_idx]))
    ...
    manifest = build_empty_mask_manifest(
        source_video_path=snapshot.video_source_path,
        resolved_video_frames_dir=str(snapshot.video_dir),
    ...
    try:
        for window_index, (window_start, window_end) in enumerate(windows):
            ...
            window_frame_paths = [Path(snapshot.video_dir) / snapshot.frame_files[idx] for idx in range(window_start, window_end + 1)]
            ...
            snapshot.video_masker.init_state(
                str(window_dir),
                online_mode=effective_online_mode,
                batch_size=effective_batch_size,
            ...
            if window_index > 0 and boundary_masks and boundary_frame_idx is not None:
                ...
                for obj_id, obj_mask in boundary_masks.items():
                    snapshot.video_masker.add_new_mask(
                        frame_idx=local_boundary_idx,
                        obj_id=int(obj_id),
                        mask=np.asarray(obj_mask).astype(bool),
                    )

            for event in snapshot.prompt_events:
                ...
                snapshot.video_masker.add_new_points_or_box(
            ...
            snapshot.video_masker.propagate_in_video(
    ...
    with state.video_state_lock:
        if current_session_path() != snapshot.session_dir:
            shutil.rmtree(pending_masks_dir, ignore_errors=True)
            raise HTTPException(status_code=409, detail="Active video session changed during propagation.")
        committed_masks_dir = commit_pending_masks_dir(snapshot.session_dir, pending_masks_dir)
        state.mask_manifest_path = str(committed_masks_dir / "manifest.json")
    ...
    return {
        ...
        "mask_manifest_path": str(snapshot.session_dir / "masks" / "manifest.json"),
        "state.mask_manifest_path": str(snapshot.session_dir / "masks" / "manifest.json"),
        "state_epoch": int(state.video_state_epoch),
    }
